...
const Npcs: FC = () => {
	...
	function renderNpcForm(npc: Npc) {
		...
		function renderLine(label: string, value: string | undefined) {
			return (
				<div className="npc-line" key={label}>
					<div>{label}:</div>
					<input type="text" name={label} value={value} onChange={onChangeField} />
		...
		return (
			<div className="npc-form">
				{attributeNames.map(n => renderLine(n, npc[n]))}
	...
	return (
		<div>
			<div>npcs</div>
			<div>
				{filterByProject ? '' : 'NOT'} filtering by project {selectedProject}
			</div>
			...
			<div className="npc-list">
				{
					npcList
						.filter(npc => !filterByProject || parseInt(npc.projectId ?? '') === selectedProject)
